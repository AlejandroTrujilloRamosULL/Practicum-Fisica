# -*- coding: utf-8 -*-
"""
Created on Wed Apr  9 08:57:59 2025
...
# código es para las estrellas que se consideran del campo HD90177a en este caso
...
# y extraemos los datos de fwhm de cada uno de ellos, que se acaban almacenando
# en files_list
...
def lnprior(theta):
    epsilon_0, L_0 = theta
    if 0.5 <= epsilon_0 <= 1 and 10 <= L_0 <= 30:
        return 0.0 
    else:
        return -np.inf
...
# donde cuando sea cero otorga el resultado de lnlike(), la probabilidad, y 
# cuando sea -infinito resulta en -infinito y el sampler descarta ese sample
...
# su probabilidad (prob), y los random states 
...
plt.plot(wave, Tokovinin(wave, best_fit_param[0], best_fit_param[1]), "--", 
         color="red", linewidth=5, label="MCMC MAP")
plt.plot(wave, opt_tokovinin, color="purple", linewidth=3,label="Predicción Tokovinin $ϵ_{LE}$")
...
plt.text(5160, 0.506, r"$ϵ_{0} = $" + f"{param[0]:.{decimals}} $\pm$ {std[0]:.{decimals_err}}" "\n" 
...
# indican la media de los valores de epsilon0 y L0 y tambien los
# mejores parámetros de ajuste (más probables)
